...
import { speakBridge, stopSpeech, isNativePlatformSync, getNativeVoices, getNativeDiagnostics } from "./native-speech.js";
import { initializePurchases, getOfferings, purchasePackage, restorePurchases, hasActivePremium, isPurchasesNative } from "./native-purchases.js";
...
function speak(text, language) {
  ...
  // Native iOS path (Capacitor): use AVSpeechSynthesizer via the plugin.
  // This sees ALL voices the user has installed in iOS Settings — including
  // Aru (Kazakh), Nannan (Shanghainese), Sin-ji (Cantonese), and every
  // dialect voice Apple withholds from Web Speech in Safari/PWAs. The
  // bridge module handles voice picking internally and is fire-and-forget.
  if (isNativePlatformSync()) {
    speakBridge(text, useLang, { rate: 0.78, pitch: 1.35, volume: 1.0 });
    return;
  }

  // Web path (PWA / Safari): use the existing window.speechSynthesis logic
  // unchanged. Voice selection via pickBestVoice handles AVOID hints,
  // premium tier preferences, accent hints, and the Chinese-family
  // fallback — all of which the native plugin doesn't need because Apple
  // curates the AVSpeechSynthesizer voice list itself.
...
function getChildPosition(child, category, language) {
  ...
  // Fall through to defaults for BOTH undefined and null. Earlier the check
  // was `pos !== undefined` which let `null` propagate — and a null couplets
  // position (left over from older saves or aborted migrations) caused
  // fetchDailyCouplets to return [], showing the user "0 cards" with no
  // explanation. Treating null and undefined identically here is safer.
...
function photoUrlForKnowledge(cardId) {
  // Look in the dedicated knowledge map first. If the entry isn't there,
  // fall back to WORD_PHOTO_URLS — many M1 set 9+ / M2 / M3 knowledge cards
  // historically had their URLs added to WORD_PHOTO_URLS by mistake (it's an
  // organizational drift from the original layout). Rather than migrating
  // 100+ entries (risk of breakage), we accept either map. WORD_PHOTO_URLS
  // is keyed by the English word for words and by card id for knowledge —
  // both happen to match cardId for our entries (e.g. "trex", "monitor").
  if (KNOWLEDGE_PHOTO_URLS[cardId]) return KNOWLEDGE_PHOTO_URLS[cardId];
  if (WORD_PHOTO_URLS[cardId]) return WORD_PHOTO_URLS[cardId];
  return `https://picsum.photos/seed/${encodeURIComponent(cardId)}/800/600`;
}
...
const WORD_PHOTO_URLS = {
  ...
  // kset12 — Parts of a Computer (handled above with computer_mouse key)
...
function Sheet({ onClose, children }) {
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.4)",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"28px 28px 0 0",width:"100%",maxWidth:480,maxHeight:"82vh",display:"flex",flexDirection:"column",paddingBottom:"env(safe-area-inset-bottom)"}}>
        {children}
      </div>
    </div>
  );
...
function AudioTestPanel({ activeChild }) {
  ...
  // Tracks whether the "show all voices on this device" diagnostic dropdown
  // is expanded. Starts closed to avoid overwhelming the panel — but it's
  // only one tap away at the very top now, easy to find.
  const [showAllVoices, setShowAllVoices] = useState(false);
  // When running native (TestFlight/App Store), AVSpeechSynthesizer exposes
  // a different (and larger) voice catalog than Web Speech API does.
  // We fetch the native voice list separately and display BOTH so the
  // diagnostic accurately reflects what the bridge can use.
  const [nativeVoices, setNativeVoices] = useState([]);
  // Diagnostic state from the native bridge — captures any errors that
  // happened during plugin import or voice query so we can surface them
  // here instead of failing silently.
  const [nativeDiag, setNativeDiag] = useState(null);
  const isNative = isNativePlatformSync();
  useEffect(() => {
    if (!isNative) return;
    let cancelled = false;
    (async () => {
      try {
        const list = await getNativeVoices();
        if (!cancelled && Array.isArray(list)) setNativeVoices(list);
      } catch {}
      try {
        const diag = await getNativeDiagnostics();
        if (!cancelled) setNativeDiag(diag);
      } catch {}
    })();
    return () => { cancelled = true; };
  }, [isNative, showAllVoices]);
  ...
  return (
    <div style={{marginTop:14,padding:"12px 14px",background:"#F0FFF4",border:"1px solid #BBF7D0",borderRadius:10}}>
      ...
      {/* Prominent diagnostic button — pinned at top so it can't be missed.
          Lets the parent see the full list of voices iOS exposes to web
          speech, definitively answering "is Nannan/Aru/etc. on the
          allowlist for this device?" Tapping toggles inline expansion. */}
      <button onClick={() => setShowAllVoices(s => !s)}
        style={{width:"100%",marginBottom:10,padding:"10px 12px",background:"#fff",border:"2px solid #1e3a8a",borderRadius:8,cursor:"pointer",fontFamily:"Nunito,sans-serif",fontWeight:800,fontSize:11,color:"#1e3a8a",textAlign:"left",display:"flex",alignItems:"center",gap:8}}>
        <span style={{fontSize:14}}>🔍</span>
        <span style={{flex:1}}>{showAllVoices ? "Hide" : "Show"} all voices on this device ({isNative ? `web: ${voices.length}, native: ${nativeVoices.length}` : voices.length})</span>
        <span>{showAllVoices ? "▲" : "▼"}</span>
      </button>
      {showAllVoices && (
        <div style={{marginBottom:10,padding:"8px",background:"#FAFAFA",border:"1px solid #ddd",borderRadius:8}}>
          {isNative && (
            <div style={{padding:"6px 8px",background:"#ECFDF5",borderRadius:6,marginBottom:8,fontFamily:"Nunito,sans-serif",fontWeight:700,fontSize:9,color:"#065f46",lineHeight:1.4}}>
              ✓ Running native iOS app — flashcards use AVSpeechSynthesizer voices below (full Apple catalog, including Nannan / Aru / Sin-ji if installed).
            </div>
          )}
          {/* Diagnostic state — shows exactly where the native bridge is
              succeeding/failing. If voices = 0, this tells us why. */}
          {isNative && nativeDiag && (
            <div style={{padding:"8px 10px",background:"#FEF3C7",border:"1px solid #FCD34D",borderRadius:6,marginBottom:8,fontFamily:"'Courier New',monospace",fontSize:9,color:"#78350F",lineHeight:1.5}}>
              <div style={{fontFamily:"Nunito,sans-serif",fontWeight:800,fontSize:10,marginBottom:4,color:"#78350F"}}>🔧 Native bridge diagnostic</div>
              <div>capacitor detected: {String(nativeDiag.capacitorDetected)}</div>
              <div>isNativePlatform: {String(nativeDiag.isNativePlatform)}</div>
              <div>plugin imported: {String(nativeDiag.pluginImported)}</div>
              {nativeDiag.pluginImportError && (
                <div style={{color:"#991B1B",fontWeight:700,wordBreak:"break-all",marginTop:2}}>import error: {nativeDiag.pluginImportError}</div>
              )}
              <div>voices queried: {String(nativeDiag.voicesQueried)}</div>
              {nativeDiag.voicesQueryError && (
                <div style={{color:"#991B1B",fontWeight:700,wordBreak:"break-all",marginTop:2}}>query error: {nativeDiag.voicesQueryError}</div>
              )}
              <div>voice count: {nativeDiag.voiceCount}</div>
            </div>
          )}
          {/* Native voice list — shown FIRST when on native, since these are
              the voices the app actually uses for flashcard reading. */}
          {isNative && (
            <div style={{marginBottom:10}}>
              <div style={{fontFamily:"Nunito,sans-serif",fontWeight:800,fontSize:10,color:"#065f46",padding:"4px 6px"}}>
                Native AVSpeechSynthesizer voices ({nativeVoices.length})
              </div>
              <div style={{maxHeight:240,overflowY:"auto"}}>
                {nativeVoices.length === 0 ? (
                  <div style={{padding:8,fontSize:10,color:"#888",fontFamily:"Nunito,sans-serif",fontWeight:700}}>
                    Native voices loading… try opening this panel again in a moment.
                  </div>
                ) : (
                  nativeVoices
                    .slice()
                    ...
                    .map((vv, idx) => (
                      <div key={(vv.voiceURI || vv.name || "v") + idx}
                        style={{display:"flex",alignItems:"center",gap:6,padding:"5px 6px",borderBottom:"1px solid #eee",fontSize:10}}>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontFamily:"Nunito,sans-serif",fontWeight:800,color:"#111"}}>{vv.name || vv.voiceURI}</div>
                          <div style={{fontFamily:"'Courier New',monospace",fontSize:9,color:"#888"}}>{vv.lang || "?"}{vv.quality ? ` · ${vv.quality}` : ""}</div>
                        </div>
                        <button onClick={async () => {
                          try { await speakBridge("Hello, this is a voice test.", vv.lang || "en-US", { rate: 0.9, pitch: 1.2 }); } catch {}
                        }}
                          style={{background:"#fff",border:"1px solid #ccc",borderRadius:50,padding:"4px 10px",cursor:"pointer",fontFamily:"Nunito,sans-serif",fontWeight:800,fontSize:9,color:"#555"}}>
                          ▶ test
                        </button>
                      </div>
                    ))
          ...
          {/* Web Speech voice list — only shown when NOT on native. On
              native, AVSpeechSynthesizer is what's actually used. */}
          {!isNative && (
          <div style={{maxHeight:280,overflowY:"auto"}}>
            {voices.length === 0 ? (
              <div style={{padding:8,fontSize:10,color:"#888",fontFamily:"Nunito,sans-serif",fontWeight:700}}>
                No voices loaded yet. Try closing this panel, waiting a moment, and reopening.
              </div>
            ) : (
              voices
                .slice()
                ...
                .map(vv => (
                  <div key={vv.name + vv.lang}
                    style={{display:"flex",alignItems:"center",gap:6,padding:"5px 6px",borderBottom:"1px solid #eee",fontSize:10}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontFamily:"Nunito,sans-serif",fontWeight:800,color:"#111"}}>{vv.name}</div>
                      <div style={{fontFamily:"'Courier New',monospace",fontSize:9,color:"#888"}}>{vv.lang}{vv.localService === false ? " · cloud" : " · local"}</div>
                    </div>
                    <button onClick={() => {
                      try {
                        window.speechSynthesis.cancel();
          ...
          {!isNative && (
            <div style={{marginTop:8,padding:"6px 8px",background:"#FFFAF0",borderRadius:6,fontFamily:"Nunito,sans-serif",fontWeight:700,fontSize:9,color:"#92400e",lineHeight:1.4}}>
              Don't see a voice you've installed in iOS Settings? Apple withholds many additional dialect voices (like Nannan for Shanghainese, Aru for Kazakh) from web pages — they only work in the native iOS app. The PWA can't access them.
            </div>
          )}
        </div>
      )}
...
function PremiumUpsell({ onClose, onSubscribed, reason }) {
  ...
  // Single price tier per interval (May 2026 — multiples upcharge removed
  // because Apple IAP doesn't support add-on pricing within a single
  // subscription product). Apple sandbox/TestFlight will charge the price
  // configured in App Store Connect for `lb_monthly` ($14.99) or
  // `lb_annual` ($119) — those values must stay in sync with PRICING below.
  const totalPrice = interval === "monthly" ? PRICING.monthly.amount : PRICING.annual.amount;
  ...
  const handleSubscribe = async () => {
    // Native (iOS via Capacitor): use RevenueCat to trigger Apple's
    // subscribe sheet. On success, persist to local state so the rest
    // of the app's premium-gating logic sees it. On user cancellation,
    // do nothing (sheet closes, upsell stays open). On error, fall back
    // gracefully — show the user a brief alert and keep the upsell
    // open. Web (PWA): skip RevenueCat entirely and persist to local
    // storage so testers using the PWA still get a working premium
    // experience for the duration of the trial.
    if (isPurchasesNative()) {
      try {
        const offerings = await getOfferings();
        const pkg = interval === "monthly" ? offerings?.monthly : offerings?.annual;
        if (!pkg) {
          // RevenueCat returned no packages — this happens if products
          // aren't fully configured, the App Store Connect sub group
          // hasn't been attached to a build yet, or the device has no
          // network. Be honest about it rather than silently faking
          // a successful purchase.
          alert("Subscription not available right now. Please try again in a moment, or contact support if this keeps happening.");
          return;
        }
        ...
        if (!result?.ok) {
          alert("Purchase didn't complete: " + (result?.error || "unknown error") + ". Please try again.");
          return;
        }
        // Success — purchase confirmed by Apple, entitlement is active
        // on RevenueCat's side. Mirror to local state so isPremium()
        // returns true immediately without waiting for the next
        // hasActivePremium() call.
        setPremium({
          tier: "family",
          interval,
          at: Date.now(),
        ...
        alert("Something went wrong with the purchase. Please try again. (" + String(err?.message || err) + ")");
        return;
      }
    }
    // Web fallback: localStorage-only "purchase" — same as before.
    // Users on the PWA can still experience the premium tier flow,
    // and we can migrate them to native IAP when they install the
    // iOS app.
    setPremium({
      tier: "family",
      interval,
    ...
    <Sheet onClose={onClose}>
      <div style={{flex:1,overflowY:"auto"}}>
        ...
        <div style={{padding:"6px 22px"}}>
          ...
          {/* Multiples add-on UI removed (May 2026): Apple IAP doesn't support
              dynamic add-on pricing within a single subscription product. To
              charge multiples families differently, we'd need 4 products
              (single + multiples × monthly + annual) and conditional logic
              based on child profile. Deferred to v1.5+ if data warrants it.
              For v1, single price tier per interval applies to all families. */}
        </div>
...
function HomeScreen({ activeChild, activeChildId, streak, onSelectCategory, language, speechOn, onLang, onToggleSpeech, onProgress, onOpenChildren, onAdjustLevel, onOpenWelcome, onEditActiveChild, onOpenParentProfile, premium }) {
  ...
  return (
    <div style={{
      ...
      // iOS native shell starts at pixel 0 by default — without this padding,
      // the top bar (Callisto avatar, profile, help, streak) hides behind
      // the iPhone's notch / Dynamic Island. env(safe-area-inset-top) is
      // 0px in Safari/web (where Safari handles the safe area itself) and
      // ~47-59px on notched iPhones running the Capacitor native shell.
      // The max() ensures we get at least 10px even on devices without a
      // notch (iPad, older iPhones).
...
function CategoryMenu({ category, activeChild, language, words, couplets, sentences, knowledge, onSelect, onOpenRoadmap, onBack }) {
  ...
  return (
    <div style={{minHeight:"100vh",background:"#fff",display:"flex",flexDirection:"column",paddingTop:"env(safe-area-inset-top)"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 18px",borderBottom:"1px solid #f5f5f5"}}>
        <button onClick={onBack} style={{background:"#f5f5f5",border:"none",borderRadius:50,width:38,height:38,fontSize:17,cursor:"pointer",color:"#555",display:"flex",alignItems:"center",justifyContent:"center"}}>←</button>
...
function SessionHeader({ onBack, index, total, sessionNum, autoPlay, onAutoPlay, extraDots, setNames }) {
  return (
    <div style={{borderBottom:"1px solid #f5f5f5", paddingTop:"env(safe-area-inset-top)"}}>
...
function ReadingSession({ childId, words, language, speechOn, sessionNum, gender, category, onBack, onComplete }) {
  ...
  // Defensive: if the session was launched but there are no cards to show
  // (e.g. fetchDailyCouplets returned empty because of a position bug, or
  // the curriculum data didn't load), show a clear error rather than a
  // blank screen with "0/0" in the header. This tells the parent exactly
  // what to do (refresh + report) instead of leaving them confused.
  if (!translating && cards.length === 0) {
    return (
      <div style={{minHeight:"100vh",background:"#fff",display:"flex",flexDirection:"column"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 18px",borderBottom:"1px solid #f5f5f5"}}>
          <button onClick={onBack} style={{background:"#f5f5f5",border:"none",borderRadius:50,width:38,height:38,fontSize:17,cursor:"pointer",color:"#555",display:"flex",alignItems:"center",justifyContent:"center"}}>←</button>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px",textAlign:"center",gap:14}}>
          <div style={{fontSize:54}}>🤔</div>
          <h2 style={{fontFamily:"'Fredoka One','Baloo 2',cursive",fontSize:22,color:"#111",margin:0,lineHeight:1.2}}>
            No {category === "couplets" ? "couplets" : category === "sentences" ? "sentences" : "cards"} to show
          </h2>
          <p style={{fontFamily:"Nunito,sans-serif",fontWeight:700,fontSize:13,color:"#666",lineHeight:1.55,maxWidth:320,margin:0}}>
            We couldn't load today's content for this language. This is usually a transient loading issue. Try going back and tapping the lesson again. If it keeps happening, use the Force Refresh button in Parent Profile.
          </p>
          <div style={{marginTop:8,padding:"8px 12px",background:"#F6F8FC",borderRadius:8,fontFamily:"'Courier New',monospace",fontSize:10,color:"#888",lineHeight:1.4}}>
            debug · category: {category || "?"} · language: {language || "?"} · words.length: {(words||[]).length}
          </div>
          <button onClick={onBack}
            style={{marginTop:8,background:RED,border:"none",borderRadius:50,padding:"12px 28px",cursor:"pointer",fontFamily:"Nunito,sans-serif",fontWeight:800,fontSize:13,color:"#fff",boxShadow:"0 4px 14px rgba(232,25,44,.25)"}}>
            ← back
          </button>
        </div>
      </div>
...
function AppInner() {
  ...
  // Initialize RevenueCat purchases SDK on app boot. On web (PWA),
  // initializePurchases() is a no-op and resolves to false — the
  // existing localStorage purchase fallback kicks in. On native iOS,
  // configure() runs against the appl_... API key and starts an
  // anonymous user session. After init, we ask RevenueCat whether the
  // user already has an active premium entitlement (cross-device
  // restore) and sync that into local state if so. This way a user who
  // subscribed on another device gets premium automatically when they
  // sign in here, without needing to tap "Restore Purchases."
  useEffect(() => {
    let cancelled = false;
    (async () => {
      ...
      try {
        const active = await hasActivePremium();
        if (cancelled) return;
        if (active === true && !isPremium()) {
          // RevenueCat says premium is active but local state doesn't
          // reflect it. Sync up — this handles cross-device restore
          // and Apple sandbox renewal scenarios.
          setPremium({
            tier: "family",
            interval: "monthly", // we don't know which from this query
            at: Date.now(),
            trialEndsAt: null,
            promoCode: null,
            dev: false,
